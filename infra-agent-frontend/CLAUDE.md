...
- `.husky/pre-commit` runs on every commit: finds staged `*.ts/tsx/js/jsx/mjs/cjs/json/css` files (`git diff --cached --diff-filter=ACMR`), runs `oxlint --fix` then `oxfmt --write` on just those files, re-stages anything they changed, and aborts the commit (non-zero exit) if oxlint has errors it couldn't auto-fix. No staged lintable files → hook is a no-op.
